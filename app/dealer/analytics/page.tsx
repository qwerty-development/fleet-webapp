...
export default function DealerAnalyticsPage() {
  ...
  const fetchAnalyticsData = useCallback(async (isRefresh = false) => {
    ...
    try {
        ...
        if (!rpcError && rpcData) {
          ...
          if (rpcData.top_viewed_cars) {
            ...
            try {
              ...
              const makeModelPatterns = rpcData.top_viewed_cars.map((car: any) =>
                `${car.make}.*${car.model}`
              ).join('|');
              ...
              const { data: carDetails, error: carDetailsError } = await supabase
                ...
                .eq('dealership_id', dealershipData.id)
                .or(
                  `make.ilike.${rpcData.top_viewed_cars[0]?.make || '%'}, model.ilike.${rpcData.top_viewed_cars[0]?.model || '%'}`
                )
              ...
              if (!carDetailsError && carDetails) {
                ...
                const enhancedCars = rpcData.top_viewed_cars.map((topCar: any) => {
                  const match = carDetails.find((detailCar: any) =>
      ...
      if (!analyticsData) {
        ...
        if (searchQuery) {
          query.or(`make.ilike.%${searchQuery}%,model.ilike.%${searchQuery}%,year.ilike.%${searchQuery}%`);
        }
  ...
  const processCarData = (cars: any[]) => {
    ...
    const avgPrice = cars.length > 0 ? cars.reduce((sum, car) => sum + (car.price || 0), 0) / cars.length : 0;
    ...
    if (soldCars.length > 0) {
      avgTimeToSell = soldCars.reduce((sum, car) => {
        const listedDate = new Date(car.listed_at);
        const soldDate = new Date(car.date_sold);
        const daysToSell = Math.floor((soldDate.getTime() - listedDate.getTime()) / (1000 * 60 * 60 * 24));
        return sum + daysToSell;
      }, 0) / soldCars.length;

      avgSalePrice = soldCars.reduce((sum, car) => sum + (car.sold_price || car.price), 0) / soldCars.length;

      priceDifference = soldCars.reduce((sum, car) => {
    ...
    const sortedCars = [...cars].sort((a, b) => (b.views || 0) - (a.views || 0));
    ...
    return {
      ...
      views_change: previousViews > 0 ? (totalViews - previousViews) / previousViews * 100 : 0,
      likes_change: previousLikes > 0 ? (totalLikes - previousLikes) / previousLikes * 100 : 0,
      sales_change: previousSales > 0 ? (totalSales - previousSales) / previousSales * 100 : 0,
  ...
  const processAnalyticsData = (data: any) => {
    ...
    if (data.category_distribution) {
      const categories = Object.entries(data.category_distribution).map(([name, value]) => ({
        name,
        value,
    ...
    if (data.inventory_status) {
      ...
      const inventoryStatus = [
        { name: 'Available', value: cars.filter((car: any) => car.status === 'available').length },
        { name: 'Pending', value: cars.filter((car: any) => car.status === 'pending').length },
        { name: 'Sold', value: cars.filter((car: any) => car.status === 'sold').length }
      ];
    ...
    if (data.top_viewed_cars) {
      ...
      const processedCars = data.top_viewed_cars.map((car: any, index: number) => ({
        ...car,
      ...
      const sorted = [...data.cars].sort((a, b) => (b.views || 0) - (a.views || 0));
    ...
    if (data.time_series_data) {
      ...
      const { viewsData, likesData, salesData } = generateTimeSeriesDataSets(overviewStats);
      setViewsData(viewsData);
      setLikesData(likesData);
      setSalesData(salesData);
    }
  ...
  const generateTimeSeriesData = (stats: any) => {
    ...
    return timeLabels.map((date, index) => {
      ...
      const views = Math.floor((stats.totalViews / timeLabels.length) * randomFactor * growth);
      const likes = Math.floor((stats.totalLikes / timeLabels.length) * randomFactor * growth);
        ...
        <div className="px-4 md:px-8 py-6 max-w-7xl mx-auto pb-16">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-400">Track performance and insights for your dealership</p>
            </div>

            <div className="flex items-center space-x-2 mt-4 md:mt-0">
              <div className="flex space-x-1 bg-gray-800 rounded-lg p-1 mr-2">
                <button
          ...
          {dealership?.subscription_end_date && getDaysUntilExpiration <= SUBSCRIPTION_WARNING_DAYS && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-6 flex items-center">
              <div className="bg-amber-500 rounded-full p-2 mr-3 flex-shrink-0">
                <ExclamationTriangleIcon className="h-5 w-5 text-white" />
              </div>
              <div>
                <h3 className="text-amber-400 font-semibold">Subscription expiring soon</h3>
          ...
          {isLoading ? (
            ...
            <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-8 text-center">
              <div className="bg-rose-500/20 rounded-full p-3 w-16 h-16 mx-auto mb-4 flex items-center justify-center">
            ...
            <>
              ...
              <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700/50 rounded-xl p-5 shadow-sm mb-8">
                <h3 className="text-lg font-semibold text-white mb-4">Inventory Metrics</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-gray-900/60 rounded-lg p-4 border border-gray-700">
                    <div className="flex items-center mb-2">
                      <div className="p-2 rounded-md bg-indigo-500/20 text-indigo-300 mr-3">
                        <BuildingStorefrontIcon className="h-5 w-5" />
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">Total Inventory</p>
                        <p className="text-xl font-semibold text-white">{inventoryMetrics.totalCars}</p>
                      </div>
                    </div>
                    <div className="flex justify-between text-sm mt-3">
                      <div>
                        <p className="text-gray-400">New</p>
                        <p className="text-white">{inventoryMetrics.newCars}</p>
                      </div>
                      <div>
                        <p className="text-gray-400">Used</p>
                        <p className="text-white">{inventoryMetrics.usedCars}</p>
                      </div>
                      <div>
                        <p className="text-gray-400">Other</p>
                        <p className="text-white">{inventoryMetrics.totalCars - inventoryMetrics.newCars - inventoryMetrics.usedCars}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-900/60 rounded-lg p-4 border border-gray-700">
                    <div className="flex items-center mb-2">
                      <div className="p-2 rounded-md bg-emerald-500/20 text-emerald-300 mr-3">
                        <CurrencyDollarIcon className="h-5 w-5" />
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">Average Price</p>
                        <p className="text-xl font-semibold text-white">{formatCurrency(inventoryMetrics.avgPrice)}</p>
                      </div>
                    </div>
                    <div className="mt-3">
                      <p className="text-sm text-gray-400">Total Inventory Value</p>
                      <p className="text-lg font-medium text-emerald-400">{formatCurrency(inventoryMetrics.totalValue)}</p>
                    </div>
                  </div>

                  <div className="bg-gray-900/60 rounded-lg p-4 border border-gray-700">
                    <div className="flex items-center mb-2">
                      <div className="p-2 rounded-md bg-amber-500/20 text-amber-300 mr-3">
                        <ShoppingCartIcon className="h-5 w-5" />
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">Sales Conversion</p>
                    ...
                    <div className="mt-3">
                      <p className="text-sm text-gray-400">Avg. Time to Sell</p>
                ...
                <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700/50 rounded-xl p-5 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-gray-900/60 rounded-lg p-4">
                        <p className="text-sm text-gray-400">Avg. Price Difference</p>
                        <p className={`text-xl font-semibold mt-1 ${
                          performanceMetrics.priceDifference >= 0 ? 'text-emerald-400' : 'text-rose-400'
                        }`}>
                    ...
                    <div className="bg-gray-900/60 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <p className="text-sm text-gray-400">Sales Timeline</p>
                      </div>
                      ...
                      <div className="space-y-3 mt-3">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center">
                            ...
                            <span className="text-gray-300">Less than 30 days</span>
                          </div>
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center">
                            ...
                            <span className="text-gray-300">30-60 days</span>
                          </div>
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center">
                            ...
                            <span className="text-gray-300">Over 60 days</span>
                          </div>
