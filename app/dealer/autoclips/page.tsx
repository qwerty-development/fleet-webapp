  ...
  useEffect(() => {
    async function fetchDealershipData() {
      ...
      try {
        ...
        // Fetch auto clips for this dealership
        ...
        console.error("Error fetching auto clips:", error);
      } finally {
        setIsLoading(false);
      }
    }
        ...
        <div className="px-4 md:px-8 py-6 max-w-7xl mx-auto">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {autoClips.map((clip) => (
                <div key={clip.id} className="bg-gray-800/80 backdrop-blur-sm border border-gray-700/50 rounded-xl overflow-hidden shadow-lg hover:shadow-xl hover:border-gray-600 transition-all duration-300">
                  <div className="h-48 overflow-hidden relative">
                    ...
                    <div className="absolute top-2 right-2 px-2 py-1 rounded text-xs text-white"
                      style={{
                        backgroundColor: clip.status === 'published' ? 'rgba(52, 211, 153, 0.8)' : 'rgba(251, 191, 36, 0.8)'
                      }}
                    >
                      {clip.status}
                    </div>
