...
export default function DealerAutoClipsPage() {
  ...
  const handleVideoSelect = (e) => {
    ...
    if (file.size > MAX_VIDEO_SIZE) {
      setFormErrors((prev) => ({
        ...prev,
        videoFile: `File is too large. Maximum size is ${
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      const fileExtension = formData.videoFile.name
        .split(".")
        .pop()
        .toLowerCase();
                ...
                {autoClips.map((clip) => (
                  ...
                  >
                    <div className="h-48 overflow-hidden relative">
                      ...
                      {clip.cars?.images && clip.cars.images.length > 0 ? (
                        <img
                          src={clip.cars.images[0]}
                          alt={`${clip.cars.make} ${clip.cars.model}`}
