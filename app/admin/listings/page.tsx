...
export default function AdminBrowseScreen() {
  ...
  const [isListingModalVisible, setIsListingModalVisible] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetchListings();
  }, [sortBy, sortOrder, filterStatus, selectedDealership, currentPage, searchQuery]);
  ...
  const handleSubmitListing = useCallback(async (formData: any) => {
  ...
  const handleDealershipChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-950 to-gray-900">
      ...
      <div className="pt-16 lg:pt-0 lg:pl-64">
        <div className="px-4 md:px-8 py-6 max-w-7xl mx-auto pb-16">
          ...
          <p className="text-gray-400">Manage all your listings, delete and change status</p>
        </div>
                ...
                <div className="relative">
                  ...
                  <div className="relative aspect-square w-full overflow-hidden bg-gray-900">
                    ...
                    <div className="w-full h-full">
                      {car.images && car.images.length > 0 ? (
                        <img 
                          src={car.images[currentImageIndexes[car.id] || 0] || '/placeholder-car.jpg'} 
                        ...
                        <button 
                          onClick={(e) => {
                            ...
                            setCurrentImageIndexes(prev => {
                              ...
                              const newIndex = (currentIndex - 1 + maxImages) % maxImages;
                    ...
                    {car.images && car.images.length > 1 && (
                      <div className="absolute bottom-3 right-3 bg-black/50 text-white px-2 py-1 rounded-md text-sm backdrop-blur-sm">
                        {(currentImageIndexes[car.id] || 0) + 1} / {car.images.length}
                      </div>
                    )}
                  ...
                  <div className="p-4 bg-gray-800/80 backdrop-blur-sm">
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <div className="flex items-center">
                        ...
                        <div className={`
                          ...
                          ${car.status === 'available' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 
                            car.status === 'pending' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 
                        ...
                        `}>
                          ...
                          <span className="capitalize font-medium">{car.status}</span>
                        </div>
                      </div>
                      <div className="flex items-center space-x-4">
                        <div className="flex items-center">
                          <EyeIcon className="h-5 w-5 text-gray-300 mr-1" />
                          <span className="text-gray-300">{car.views || 0}</span>
                        </div>
                        <div className="flex items-center">
                          <HeartIcon className="h-5 w-5 text-gray-300 mr-1" />
                          <span className="text-gray-300">{car.likes || 0}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <button 
                        onClick={() => {
                          const newStatus = car.status === 'available' ? 'pending' : 'available';
                          
                          // Confirm before changing status
                          if (confirm(`Change status from ${car.status} to ${newStatus}?`)) {
                        ...
                        className={`
                          ...
                          ${car.status === 'sold' ? 'bg-gray-700 text-gray-300 cursor-not-allowed' : 
                            car.status === 'available' ? 'bg-amber-600/90 hover:bg-amber-600 text-white' : 'bg-emerald-600/90 hover:bg-emerald-600 text-white'} 
                      ...
                      >
                        {car.status === 'sold' ? (
                          ...
                          <>
                            ...
                            {car.status === 'available' ? 'Mark as Pending' : 'Mark as Available'}
                          </>
                        )}
                      </button>
