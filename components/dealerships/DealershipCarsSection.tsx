...
const DealershipCarsSection: React.FC<DealershipCarsSectionProps> = ({
  dealershipID,
  currentCarId,
}) => {
  ...
  useEffect(() => {
    ...
    console.log(
      "Fetching cars for dealership ID:",
      dealershipID,
      "Type:",
      typeof dealershipID
    );
    ...
    const fetchCars = async () => {
      ...
      try {
        ...
        console.log(
          `Found ${data?.length || 0} cars for dealership ID ${dealershipID}`
        );
        setCars(data || []);
        setLoading(false);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center p-8">
        ...
        <span className="ml-2 text-gray-700">Loading cars...</span>
      </div>
    );
  }
  ...
  if (!cars.length) {
    return (
      <p className="mt-8 text-center text-gray-500">
        No cars available from this dealership at the moment.
      </p>
    );
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {cars.map((car) => (
          <Link key={car.id} href={`/cars/${car.id}`}>
            <div className="bg-white border border-gray-200 rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden cursor-pointer h-full">
              {/* Car Image */}
