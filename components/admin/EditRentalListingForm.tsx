...
interface EditRentalListingFormProps {
  listing: Car;
  onClose: () => void;
...
const VEHICLE_FEATURES = [
  ...
  { id: 'keyless_start', label: 'Keyless Start' },
  { id: 'power_mirrors', label: 'Power Mirrors' },
  ...
  { id: 'third_row_seats', label: 'Third Row Seats' },
  { id: 'parking_sensors', label: 'Parking Sensors' },
  { id: 'lane_assist', label: 'Lane Departure Warning' },
  { id: 'blind_spot', label: 'Blind Spot Monitoring' },
...
const EditRentalListingForm: React.FC<EditRentalListingFormProps> = ({
  listing,
  onClose,
  onSubmit,
}) => {
  ...
  // Initialize form data state - ONLY fields that exist in cars_rent table
  const [formData, setFormData] = useState({
    ...
    type: listing.type || "Benzine",
    category: listing.category || "Sedan",
    rental_period: (listing as any).rental_period || "daily",
    trim: listing.trim || "",
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      // Create a data object with ONLY fields that exist in cars_rent table
      const dataToSubmit = {
        make: formData.make,
        ...
        trim: typeof formData.trim === 'string' ? (formData.trim.trim() || null) : null,
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-gray-800 z-10 px-6 py-4 border-b border-gray-700 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-white">Edit Rental Listing</h2>
            <p className="text-sm text-purple-400 mt-1">Cars for Rent</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label htmlFor="rental_period" className="block text-sm font-medium text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-white"
                placeholder="e.g. Benzine, Diesel, Electric"
              />
            </div>
            
            <div>
              <label htmlFor="trim" className="block text-sm font-medium text-gray-300 mb-1">
                <div className="flex items-center">
...
export default EditRentalListingForm;
